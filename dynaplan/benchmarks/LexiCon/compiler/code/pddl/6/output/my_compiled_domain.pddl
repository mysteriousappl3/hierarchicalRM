(define (domain mytrajectoryconstraintsremover_minigrid_problem-domain)
 ...
 (not (blocked ?door ?o ?room1))))
  :effect (and (at_ ?door)(forall (?o - object_) (when (at_ ?o) (not (at_ ?o))))(forall (?d - door) (when (at_ ?d) (not (at_ ?d)))) (when (and (not (emptyhands)) (= ?door green_door_1)) (hold_0)) (increase (total-cost) 1)))
 (:action gotoroom
 ...
 (:action pick
  ...
  :effect (and (not (at_ ?obj)) (not (emptyhands)) (carrying ?obj) (not (objectinroom ?obj ?room))(forall (?d - door) (when (blocked ?d ?obj ?room) (not (blocked ?d ?obj ?room)))) (when (at_ green_door_1) (hold_0)) (increase (total-cost) 1)))
 (:action drop
  :parameters ( ?obj - object_ ?room - room)
