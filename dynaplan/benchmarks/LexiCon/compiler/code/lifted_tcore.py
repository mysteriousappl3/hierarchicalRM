...
def solve_unconstrained(problem):
    try:
        with OneshotPlanner(
            name="symk", optimality_guarantee=OptimalityGuarantee.SOLVED_OPTIMALLY
        ) as planner:
        ...
        raise RuntimeError(f"Error while solving unconstrained planning problem: {e}") from e
...
def solve_compiled_lifted(compiled_problem):
    try:
        ...
        raise RuntimeError(f"Error while solving constrained planning problem: {e}") from e
...
def compile_tcore(problem):
    try:
        ...
        raise RuntimeError(f"Error while compiling unconstrained planning problem: {e}") from e
...
def solve_compiled_grounded(tcore_result):
    ...
    try:
        with OneshotPlanner(
            ...
            res = planner.solve(problem)
            if res.status == up.engines.results.PlanGenerationResultStatus.SOLVED_OPTIMALLY:
                lifted_plan = res.plan.replace_action_instances(
                    tcore_result.map_back_action_instance
                )
                return tcore_result, res, lifted_plan
            else:
                return tcore_result, res, None
    except Exception as e:
        raise RuntimeError(f"Error while solving constrained planning problem: {e}") from e
...
def get_blocksworld_problem(num_blocks=5, seed=None):
    ...
    for _ in range(100):
        ...
        print(random_block)
        print(problem.initial_value(clear(random_block)))
        if problem.initial_value(clear(random_block)).is_false():
            break
    print(random_block)
    constraint = Sometime(FluentExp(clear, [random_block]))
    print(constraint)
...
class TrajectoryConstraintsRemover(engines.engine.Engine, CompilerMixin):
    ...
    @property
    def name(self):
        return "MyTrajectoryConstraintsRemover"
    ...
    def _compile(
        ...
        print(f"New problem name: {new_problem.name}")
    ...
    def _gamma_substitution(self, env, literal, action):
        ...
        print(f"R(f, a) = {env.expression_manager.Or(gamma1, conjunction)}")
    ...
    def _unification(self, eff_fluentexp, lit_fluentexp, x_vars):
        ...
        if eff_fluentexp.fluent() != lit_fluentexp.fluent() or len(eff_fluentexp.args) != len(
            ...
            print("Different fluent: continue")
            return None
        xi = dict()
        for i in range(len(eff_fluentexp.args)):
            ...
            if f1arg.is_object_exp() and f2arg.is_object_exp() and f1arg != f2arg:
                ...
                return None
            # Subtyping is checked it each subcase
            # elif f1arg._content.payload.type != f2arg._content.payload.type:
            # print(f"Args: {f1arg} and {f2arg} have different types")
            # return None
            # elif (
            # f1arg.is_object_exp()
            # and f2arg.is_parameter_exp()
            # and f1arg.object().type.is_subtype(f2arg.parameter().type)
            # ):
            # Does this ever happen?
            # print(f"Object: {f1arg} can be unified with Constraint Parameter: {f2arg}")
            # xi[f1arg] = f2arg
            elif (
    ...
    def _gamma(self, env, literal, action):
        ...
        for eff in action.effects:
            ...
            for param in uni:
                """
                if cond.is_true():
                    print(f"cond is {cond.is_true()}")
                    if param.is_parameter_exp() and param.parameter() in x_vars:
                        return Equals(param, uni[param])
                    else:
...
if __name__ == "__main__":
    ...
    constrained_problem, unconstrained_problem = get_doubleblocksworld_problem(test_case=test_case)

    unconstrained_start_time = time.time()
    unconstrained_plan = solve_unconstrained(unconstrained_problem)
    print(f"Unconstrained Planning Time: {time.time() - unconstrained_start_time}")
    print(unconstrained_plan)

    # Use mycompiler
    my_start_time = time.time()
    mycompiler = TrajectoryConstraintsRemover()
    compiled = mycompiler._compile(
        constrained_problem, CompilationKind.TRAJECTORY_CONSTRAINTS_REMOVING
    )
    print(f"My Compiler Time: {time.time() - my_start_time}")
    writer = PDDLWriter(compiled)
    writer.write_domain(f"pddl/{test_case}/output/my_compiled_domain.pddl")
    writer.write_problem(f"pddl/{test_case}/output/my_compiled_problem.pddl")
    # print(compiled)
    # Solve problem
    my_solve_start_time = time.time()
    myres = solve_compiled_lifted(compiled)
    print(myres)
    print(f"Symk Lifted Planning Time: {time.time() - my_solve_start_time}")
    print()
    if myres.plan:
        writer.write_plan(myres.plan, f"pddl/{test_case}/output/my_plan")
    # use TCORE to compile, and then solve
    tcore_start_time = time.time()
    compiled_grounded_result = compile_tcore(constrained_problem)
    print(f"TCORE Compilation Time: {time.time() - tcore_start_time}")
    writer = PDDLWriter(compiled_grounded_result.problem)
    writer.write_domain(f"pddl/{test_case}/output/compiled_domain.pddl")
    writer.write_problem(f"pddl/{test_case}/output/compiled_problem.pddl")
    tcore_solve_start_time = time.time()
    tcr_result, res, lifted_plan = solve_compiled_grounded(compiled_grounded_result)
    print(f"Symk Grounded Planning Time: {time.time() - tcore_solve_start_time}")
    writer.write_plan(res.plan, f"pddl/{test_case}/output/plan")
    # print(tcr_result)
    # print(res)
    print(lifted_plan)
