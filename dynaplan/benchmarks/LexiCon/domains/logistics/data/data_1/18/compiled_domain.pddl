(define (domain liftedtcore_logistics-domain)
 ...
 (:action loadtruck
  ...
  :effect (and (not (at_ ?obj_1 ?loc)) (in ?obj_1 ?truck) (when (or (in p2 a1) (and (at_ p2 l2_5) (not (and (= ?obj_1 p2) (= ?loc l2_5))))) (hold_0)) (increase (total-cost) 1)))
 (:action loadairplane
  :parameters ( ?obj_1 - package ?airplane - airplane ?loc - location)
  :precondition (and (at_ ?obj_1 ?loc) (at_ ?airplane ?loc))
  :effect (and (not (at_ ?obj_1 ?loc)) (in ?obj_1 ?airplane) (when (or (and (= ?obj_1 p2) (= ?airplane a1)) (in p2 a1) (and (at_ p2 l2_5) (not (and (= ?obj_1 p2) (= ?loc l2_5))))) (hold_0)) (increase (total-cost) 1)))
 (:action unloadtruck
  :parameters ( ?obj - obj ?truck - truck ?loc - location)
  :precondition (and (at_ ?truck ?loc) (in ?obj ?truck))
  :effect (and (not (in ?obj ?truck)) (at_ ?obj ?loc) (when (or (in p2 a1) (and (= ?obj p2) (= ?loc l2_5)) (at_ p2 l2_5)) (hold_0)) (increase (total-cost) 1)))
 (:action unloadairplane
  :parameters ( ?obj - obj ?airplane - airplane ?loc - location)
  :precondition (and (in ?obj ?airplane) (at_ ?airplane ?loc))
  :effect (and (not (in ?obj ?airplane)) (at_ ?obj ?loc) (when (or (and (in p2 a1) (not (and (= ?obj p2) (= ?airplane a1)))) (and (= ?obj p2) (= ?loc l2_5)) (at_ p2 l2_5)) (hold_0)) (increase (total-cost) 1)))
 (:action drivetruck
