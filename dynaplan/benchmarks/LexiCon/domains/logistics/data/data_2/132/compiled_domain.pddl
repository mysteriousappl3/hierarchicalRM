(define (domain liftedtcore_logistics-domain)
 ...
 (:action loadtruck
  ...
  :effect (and (not (at_ ?obj_1 ?loc)) (in ?obj_1 ?truck) (when (or (and (= ?obj_1 p1) (= ?truck t1)) (in p1 t1)) (seen_psi_1)) (increase (total-cost) 1)))
 (:action loadairplane
  :parameters ( ?obj_1 - package ?airplane - airplane ?loc - location)
  :precondition (and (at_ ?obj_1 ?loc) (at_ ?airplane ?loc) (or (not (or (and (= ?obj_1 p1) (= ?airplane a2)) (in p1 a2))) (seen_psi_1)))
  :effect (and (not (at_ ?obj_1 ?loc)) (in ?obj_1 ?airplane) (when (or (and (= ?obj_1 p1) (= ?airplane a2)) (in p1 a2)) (hold_0)) (when (or (and (= ?obj_1 p1) (= ?airplane a1)) (in p1 a1)) (hold_2)) (increase (total-cost) 1)))
 (:action unloadtruck
  :parameters ( ?obj - obj ?truck - truck ?loc - location)
  :precondition (and (at_ ?truck ?loc) (in ?obj ?truck))
  :effect (and (not (in ?obj ?truck)) (at_ ?obj ?loc) (when (and (in p1 t1) (not (and (= ?obj p1) (= ?truck t1)))) (seen_psi_1)) (increase (total-cost) 1)))
 (:action unloadairplane
  :parameters ( ?obj - obj ?airplane - airplane ?loc - location)
  :precondition (and (in ?obj ?airplane) (at_ ?airplane ?loc) (or (not (and (in p1 a2) (not (and (= ?obj p1) (= ?airplane a2))))) (seen_psi_1)))
  :effect (and (not (in ?obj ?airplane)) (at_ ?obj ?loc) (when (and (in p1 a2) (not (and (= ?obj p1) (= ?airplane a2)))) (hold_0)) (when (and (in p1 a1) (not (and (= ?obj p1) (= ?airplane a1)))) (hold_2)) (increase (total-cost) 1)))
 (:action drivetruck
