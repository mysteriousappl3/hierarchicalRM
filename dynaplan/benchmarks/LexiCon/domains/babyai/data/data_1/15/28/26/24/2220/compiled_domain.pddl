(define (domain liftedtcore_minigrid-domain)
 ...
 (:action drop
  ...
  :effect (and (not (carrying ?obj)) (at_ ?obj) (not (at_ empty)) (emptyhands) (objectinroom ?obj ?room) (when (locked purple_door_2) (not (hold_1))) (increase (total-cost) 1)))
 (:action toggle
  :parameters ( ?door - door)
  :precondition (and (at_ ?door))
  :effect (and (when (not (locked ?door)) (locked ?door)) (when (locked ?door) (not (locked ?door))) (when (or (and (not (locked ?door)) (= ?door purple_door_2)) (and (locked purple_door_2) (not (and (locked ?door) (= ?door purple_door_2))))) (hold_0)) (when (and (or (and (not (locked ?door)) (= ?door purple_door_2)) (and (locked purple_door_2) (not (and (locked ?door) (= ?door purple_door_2))))) (emptyhands)) (not (hold_1))) (increase (total-cost) 1)))
)
