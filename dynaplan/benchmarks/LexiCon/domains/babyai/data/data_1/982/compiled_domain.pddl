(define (domain liftedtcore_minigrid-domain)
 ...
 (not (blocked ?door ?o ?room1))) (or (not (= ?door purple_door_1)) (seen_psi_1)))
  :effect (and(forall (?o - object_) (when (at_ ?o) (not (at_ ?o))))(forall (?d - door) (when (at_ ?d) (not (at_ ?d)))) (at_ ?door) (when (= ?door purple_door_1) (hold_0)) (increase (total-cost) 1)))
 (:action gotoroom
 ...
 (:action pick
  ...
  :effect (and (not (at_ ?obj)) (not (emptyhands)) (carrying ?obj) (not (objectinroom ?obj ?room))(forall (?d - door) (when (blocked ?d ?obj ?room) (not (blocked ?d ?obj ?room)))) (seen_psi_1) (increase (total-cost) 1)))
 (:action drop
  :parameters ( ?obj - object_ ?room - room)
