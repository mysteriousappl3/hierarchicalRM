(define (domain liftedtcore_minigrid-domain)
 ...
 (:action pick
  ...
  :effect (and (not (at_ ?obj)) (not (emptyhands)) (carrying ?obj) (not (objectinroom ?obj ?room))(forall (?d - door) (when (blocked ?d ?obj ?room) (not (blocked ?d ?obj ?room)))) (when (and (locked purple_door_1) (not (and (objectinroom yellow_ball_1 room_4) (not (and (= ?obj yellow_ball_1) (= ?room room_4)))))) (not (hold_1))) (when (and (objectinroom yellow_ball_1 room_4) (not (and (= ?obj yellow_ball_1) (= ?room room_4)))) (hold_1)) (increase (total-cost) 1)))
 (:action drop
  :parameters ( ?obj - object_ ?room - room)
  :precondition (and (agentinroom ?room) (carrying ?obj) (not (emptyhands)) (at_ empty))
  :effect (and (not (carrying ?obj)) (at_ ?obj) (not (at_ empty)) (emptyhands) (objectinroom ?obj ?room) (when (and (locked purple_door_1) (not (or (and (= ?obj yellow_ball_1) (= ?room room_4)) (objectinroom yellow_ball_1 room_4)))) (not (hold_1))) (when (or (and (= ?obj yellow_ball_1) (= ?room room_4)) (objectinroom yellow_ball_1 room_4)) (hold_1)) (increase (total-cost) 1)))
 (:action toggle
  :parameters ( ?door - door)
  :precondition (and (at_ ?door))
  :effect (and (when (not (locked ?door)) (locked ?door)) (when (locked ?door) (not (locked ?door))) (when (or (and (not (locked ?door)) (= ?door purple_door_1)) (and (locked purple_door_1) (not (and (locked ?door) (= ?door purple_door_1))))) (hold_0)) (when (and (or (and (not (locked ?door)) (= ?door purple_door_1)) (and (locked purple_door_1) (not (and (locked ?door) (= ?door purple_door_1))))) (not (objectinroom yellow_ball_1 room_4))) (not (hold_1))) (increase (total-cost) 1)))
)
