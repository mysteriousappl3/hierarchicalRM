(define (domain liftedtcore_minigrid-domain)
 ...
 (not (blocked ?door ?o ?room1))))
  :effect (and(forall (?o - object_) (when (at_ ?o) (not (at_ ?o))))(forall (?d - door) (when (at_ ?d) (not (at_ ?d)))) (at_ ?door) (when (= ?door green_door_1) (seen_psi_1)) (increase (total-cost) 1)))
 (:action gotoroom
 ...
 (:action toggle
  ...
  :precondition (and (at_ ?door) (or (and (not (locked ?door)) (= ?door blue_door_2)) (and (locked blue_door_2) (not (and (locked ?door) (= ?door blue_door_2)))) (seen_psi_1)))
  :effect (and (when (not (locked ?door)) (locked ?door)) (when (locked ?door) (not (locked ?door))) (when (not (or (and (not (locked ?door)) (= ?door blue_door_2)) (and (locked blue_door_2) (not (and (locked ?door) (= ?door blue_door_2)))))) (hold_0)) (increase (total-cost) 1)))
)
