(define (domain liftedtcore_minigrid-domain)
 ...
 (:action gotoobject
  ...
  :effect (and(forall (?o - object_) (when (at_ ?o) (not (at_ ?o))))(forall (?d - door) (when (at_ ?d) (not (at_ ?d)))) (at_ ?obj) (when (= ?obj blue_box_2) (hold_1)) (increase (total-cost) 1)))
 (:action gotoempty
  :parameters ()
 ...
 (:action pick
  ...
  :effect (and (not (at_ ?obj)) (not (emptyhands)) (carrying ?obj) (not (objectinroom ?obj ?room))(forall (?d - door) (when (blocked ?d ?obj ?room) (not (blocked ?d ?obj ?room)))) (when (and (objectinroom blue_box_2 room_1) (not (and (= ?obj blue_box_2) (= ?room room_1)))) (hold_0)) (when (and (at_ blue_box_2) (not (= ?obj blue_box_2))) (hold_1)) (increase (total-cost) 1)))
 (:action drop
  :parameters ( ?obj - object_ ?room - room)
  :precondition (and (agentinroom ?room) (carrying ?obj) (not (emptyhands)) (at_ empty))
  :effect (and (not (carrying ?obj)) (at_ ?obj) (not (at_ empty)) (emptyhands) (objectinroom ?obj ?room) (when (or (and (= ?obj blue_box_2) (= ?room room_1)) (objectinroom blue_box_2 room_1)) (hold_0)) (when (or (= ?obj blue_box_2) (at_ blue_box_2)) (hold_1)) (increase (total-cost) 1)))
 (:action toggle
