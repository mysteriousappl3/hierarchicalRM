(define (domain liftedtcore_minigrid-domain)
 ...
 (:action pick
  ...
  :effect (and (not (at_ ?obj)) (not (emptyhands)) (carrying ?obj) (not (objectinroom ?obj ?room))(forall (?d - door) (when (blocked ?d ?obj ?room) (not (blocked ?d ?obj ?room)))) (when (or (= ?obj blue_box_1) (carrying blue_box_1)) (seen_psi_1)) (when (and (objectinroom red_ball_1 room_4) (not (and (= ?obj red_ball_1) (= ?room room_4)))) (hold_2)) (increase (total-cost) 1)))
 (:action drop
  :parameters ( ?obj - object_ ?room - room)
  :precondition (and (agentinroom ?room) (carrying ?obj) (not (emptyhands)) (at_ empty))
  :effect (and (not (carrying ?obj)) (at_ ?obj) (not (at_ empty)) (emptyhands) (objectinroom ?obj ?room) (when (and (carrying blue_box_1) (not (= ?obj blue_box_1))) (seen_psi_1)) (when (or (and (= ?obj red_ball_1) (= ?room room_4)) (objectinroom red_ball_1 room_4)) (hold_2)) (increase (total-cost) 1)))
 (:action toggle
