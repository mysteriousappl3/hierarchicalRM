(define (domain liftedtcore_minigrid-domain)
 ...
 (:action gotoobject
  ...
  :effect (and(forall (?o - object_) (when (at_ ?o) (not (at_ ?o))))(forall (?d - door) (when (at_ ?d) (not (at_ ?d)))) (at_ ?obj) (when (= ?obj blue_box_1) (hold_0)) (increase (total-cost) 1)))
 (:action gotoempty
  :parameters ()
 ...
 (:action gotoroom
  ...
  :effect (and (not (agentinroom ?room1)) (agentinroom ?room2) (visited ?room2)(forall (?o - object_) (when (at_ ?o) (not (at_ ?o))))(forall (?d - door) (when (at_ ?d) (not (at_ ?d)))) (when (and (or (= ?room2 room_3) (and (agentinroom room_3) (not (= ?room1 room_3)))) (not (emptyhands))) (hold_3)) (increase (total-cost) 1)))
 (:action pick
  :parameters ( ?obj - object_ ?room - room)
  :precondition (and (agentinroom ?room) (objectinroom ?obj ?room) (at_ ?obj) (emptyhands))
  :effect (and (not (at_ ?obj)) (not (emptyhands)) (carrying ?obj) (not (objectinroom ?obj ?room))(forall (?d - door) (when (blocked ?d ?obj ?room) (not (blocked ?d ?obj ?room)))) (when (and (at_ blue_box_1) (not (= ?obj blue_box_1))) (hold_0)) (when (and (objectinroom red_ball_1 room_3) (not (and (= ?obj red_ball_1) (= ?room room_3)))) (seen_psi_2)) (when (agentinroom room_3) (hold_3)) (increase (total-cost) 1)))
 (:action drop
  :parameters ( ?obj - object_ ?room - room)
  :precondition (and (agentinroom ?room) (carrying ?obj) (not (emptyhands)) (at_ empty))
  :effect (and (not (carrying ?obj)) (at_ ?obj) (not (at_ empty)) (emptyhands) (objectinroom ?obj ?room) (when (or (= ?obj blue_box_1) (at_ blue_box_1)) (hold_0)) (when (or (and (= ?obj red_ball_1) (= ?room room_3)) (objectinroom red_ball_1 room_3)) (seen_psi_2)) (increase (total-cost) 1)))
 (:action toggle
