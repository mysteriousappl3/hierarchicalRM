(define (domain liftedtcore_minigrid-domain)
 ...
 (:action gotoroom
  ...
  :precondition (and (agentinroom ?room1) (adjacentrooms ?room1 ?room2 ?door) (not (locked ?door)) (or (not (or (= ?room2 room_2) (and (agentinroom room_2) (not (= ?room1 room_2))))) (seen_psi_2)))
 ...
 (:action pick
  ...
  :effect (and (not (at_ ?obj)) (not (emptyhands)) (carrying ?obj) (not (objectinroom ?obj ?room))(forall (?d - door) (when (blocked ?d ?obj ?room) (not (blocked ?d ?obj ?room)))) (hold_0) (seen_psi_2) (increase (total-cost) 1)))
 (:action drop
  :parameters ( ?obj - object_ ?room - room)
  :precondition (and (agentinroom ?room) (carrying ?obj) (not (emptyhands)) (at_ empty))
  :effect (and (not (carrying ?obj)) (at_ ?obj) (not (at_ empty)) (emptyhands) (objectinroom ?obj ?room) (when (or (and (= ?obj purple_box_1) (= ?room room_2)) (objectinroom purple_box_1 room_2)) (hold_0)) (when (or (and (= ?obj red_box_1) (= ?room room_4)) (objectinroom red_box_1 room_4)) (seen_psi_2)) (increase (total-cost) 1)))
 (:action toggle
