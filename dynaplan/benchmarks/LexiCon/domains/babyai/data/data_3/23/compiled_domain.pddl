(define (domain liftedtcore_minigrid-domain)
 ...
 (:constants
   empty - empty_0
   grey_door_1 red_door_1 blue_door_1 - door
   grey_ball_1 - ball
   room_2 - room
 )
 ...
 (:action pick
  ...
  :effect (and (not (at_ ?obj)) (not (emptyhands)) (carrying ?obj) (not (objectinroom ?obj ?room))(forall (?d - door) (when (blocked ?d ?obj ?room) (not (blocked ?d ?obj ?room)))) (when (and (objectinroom grey_ball_1 room_2) (not (and (= ?obj grey_ball_1) (= ?room room_2)))) (seen_psi_3)) (increase (total-cost) 1)))
 (:action drop
  :parameters ( ?obj - object_ ?room - room)
  :precondition (and (agentinroom ?room) (carrying ?obj) (not (emptyhands)) (at_ empty))
  :effect (and (not (carrying ?obj)) (at_ ?obj) (not (at_ empty)) (emptyhands) (objectinroom ?obj ?room) (when (or (and (= ?obj grey_ball_1) (= ?room room_2)) (objectinroom grey_ball_1 room_2)) (seen_psi_3)) (increase (total-cost) 1)))
 (:action toggle
