(define (domain liftedtcore_minigrid-domain)
 ...
 (:action gotoobject
  ...
  :effect (and(forall (?o - object_) (when (at_ ?o) (not (at_ ?o))))(forall (?d - door) (when (at_ ?d) (not (at_ ?d)))) (at_ ?obj) (when (= ?obj purple_ball_1) (seen_psi_1)) (when (= ?obj purple_ball_2) (hold_5)) (increase (total-cost) 1)))
 (:action gotoempty
  :parameters ()
 ...
 (:action gotoroom
  ...
  :effect (and (not (agentinroom ?room1)) (agentinroom ?room2) (visited ?room2)(forall (?o - object_) (when (at_ ?o) (not (at_ ?o))))(forall (?d - door) (when (at_ ?d) (not (at_ ?d)))) (when (or (= ?room2 room_4) (and (agentinroom room_4) (not (= ?room1 room_4)))) (hold_3)) (increase (total-cost) 1)))
 (:action pick
  :parameters ( ?obj - object_ ?room - room)
  :precondition (and (agentinroom ?room) (objectinroom ?obj ?room) (at_ ?obj) (emptyhands) (seen_psi_1))
  :effect (and (not (at_ ?obj)) (not (emptyhands)) (carrying ?obj) (not (objectinroom ?obj ?room))(forall (?d - door) (when (blocked ?d ?obj ?room) (not (blocked ?d ?obj ?room)))) (hold_0) (when (and (at_ purple_ball_1) (not (= ?obj purple_ball_1))) (seen_psi_1)) (when (and (objectinroom grey_box_1 room_4) (not (and (= ?obj grey_box_1) (= ?room room_4)))) (hold_2)) (when (and (not (locked grey_door_3)) (or (= ?obj yellow_ball_1) (carrying yellow_ball_1))) (hold_4)) (when (and (at_ purple_ball_2) (not (= ?obj purple_ball_2))) (hold_5)) (increase (total-cost) 1)))
 (:action drop
  :parameters ( ?obj - object_ ?room - room)
  :precondition (and (agentinroom ?room) (carrying ?obj) (not (emptyhands)) (at_ empty))
  :effect (and (not (carrying ?obj)) (at_ ?obj) (not (at_ empty)) (emptyhands) (objectinroom ?obj ?room) (when (or (= ?obj purple_ball_1) (at_ purple_ball_1)) (seen_psi_1)) (when (or (and (= ?obj grey_box_1) (= ?room room_4)) (objectinroom grey_box_1 room_4)) (hold_2)) (when (and (not (locked grey_door_3)) (carrying yellow_ball_1) (not (= ?obj yellow_ball_1))) (hold_4)) (when (or (= ?obj purple_ball_2) (at_ purple_ball_2)) (hold_5)) (increase (total-cost) 1)))
 (:action toggle
  :parameters ( ?door - door)
  :precondition (and (at_ ?door))
  :effect (and (when (not (locked ?door)) (locked ?door)) (when (locked ?door) (not (locked ?door))) (when (and (not (or (and (not (locked ?door)) (= ?door grey_door_3)) (and (locked grey_door_3) (not (and (locked ?door) (= ?door grey_door_3)))))) (carrying yellow_ball_1)) (hold_4)) (increase (total-cost) 1)))
)
