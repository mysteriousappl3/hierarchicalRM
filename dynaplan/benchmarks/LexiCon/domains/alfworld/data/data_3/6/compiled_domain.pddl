(define (domain liftedtcore_alfred-domain)
 ...
 (:action inventory
  :parameters ( ?a - agent)
  :effect (and (checked ?a) (when (or (checked fridge_5) (= ?a agent1) (checked agent1)) (hold_2)) (increase (total-cost) 1)))
 (:action examinereceptacle
  :parameters ( ?a - agent ?r - receptacle)
  :precondition (and (exists (?l_0 - location)
 (and (atlocation ?a ?l_0) (receptacleatlocation ?r ?l_0))))
  :effect (and (checked ?r) (when (or (= ?r fridge_5) (checked fridge_5) (checked agent1)) (hold_2)) (increase (total-cost) 1)))
 ...
 (:action openobject
  ...
  :effect (and (opened ?r) (checked ?r) (when (or (= ?r fridge_5) (checked fridge_5) (checked agent1)) (hold_2)) (increase (total-cost) 1)))
 (:action closeobject
  :parameters ( ?a - agent ?l - location ?r - receptacle)
 ...
 (:action pickupobject
  ...
  :effect (and (not (inreceptacle ?o ?r)) (holds ?a ?o) (holdsany ?a) (not (objectatlocation ?o ?l)) (when (and (or (and (= ?a agent1) (= ?o candle_2)) (holds agent1 candle_2)) (or (= ?a agent1) (holdsany agent1))) (hold_0)) (when (or (and (= ?a agent1) (= ?o tennisracket_1)) (holds agent1 tennisracket_1)) (hold_1)) (increase (total-cost) 1)))
 (:action putobject
  ...
  :effect (and (inreceptacle ?o ?r) (objectatlocation ?o ?l) (not (holds ?a ?o)) (not (holdsany ?a)) (when (and (holds agent1 candle_2) (not (and (= ?a agent1) (= ?o candle_2))) (holdsany agent1) (not (= ?a agent1))) (hold_0)) (when (and (holds agent1 tennisracket_1) (not (and (= ?a agent1) (= ?o tennisracket_1)))) (hold_1)) (increase (total-cost) 1)))
 (:action cleanobject
