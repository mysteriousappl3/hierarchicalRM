(define (domain liftedtcore_alfred-domain)
 ...
 (and (atlocation ?a ?l_0) (receptacleatlocation ?r ?l_0))))
  :effect (and (checked ?r) (when (and (or (= ?r fridge_7) (checked fridge_7)) (atlocation agent1 location1)) (hold_0)) (increase (total-cost) 1)))
 ...
 (:action gotolocation
  ...
  :effect (and (not (atlocation ?a ?lstart)) (atlocation ?a ?lend) (when (and (checked fridge_7) (or (and (= ?a agent1) (= ?lend location1)) (and (atlocation agent1 location1) (not (and (= ?a agent1) (= ?lstart location1)))))) (hold_0)) (increase (total-cost) 1)))
 (:action openobject
  :parameters ( ?a - agent ?l - location ?r - receptacle)
  :precondition (and (openable ?r) (atlocation ?a ?l) (receptacleatlocation ?r ?l) (not (opened ?r)))
  :effect (and (opened ?r) (checked ?r) (when (and (or (= ?r fridge_7) (checked fridge_7)) (atlocation agent1 location1)) (hold_0)) (increase (total-cost) 1)))
 (:action closeobject
  :parameters ( ?a - agent ?l - location ?r - receptacle)
