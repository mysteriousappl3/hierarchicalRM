(define (domain liftedtcore_alfred-domain)
 ...
 (and (atlocation ?a ?l_0) (receptacleatlocation ?r ?l_0))))
  :effect (and (checked ?r) (when (or (atlocation agent1 location1) (= ?r drawer_3) (checked drawer_3)) (seen_psi_1)) (increase (total-cost) 1)))
 ...
 (:action gotolocation
  ...
  :precondition (and (atlocation ?a ?lstart) (or (not (or (and (= ?a agent1) (= ?lend location7)) (and (atlocation agent1 location7) (not (and (= ?a agent1) (= ?lstart location7)))))) (seen_psi_1)))
  :effect (and (not (atlocation ?a ?lstart)) (atlocation ?a ?lend) (when (or (and (= ?a agent1) (= ?lend location7)) (and (atlocation agent1 location7) (not (and (= ?a agent1) (= ?lstart location7))))) (hold_0)) (when (or (and (= ?a agent1) (= ?lend location1)) (and (atlocation agent1 location1) (not (and (= ?a agent1) (= ?lstart location1)))) (checked drawer_3)) (seen_psi_1)) (increase (total-cost) 1)))
 (:action openobject
  :parameters ( ?a - agent ?l - location ?r - receptacle)
  :precondition (and (openable ?r) (atlocation ?a ?l) (receptacleatlocation ?r ?l) (not (opened ?r)))
  :effect (and (opened ?r) (checked ?r) (when (or (atlocation agent1 location1) (= ?r drawer_3) (checked drawer_3)) (seen_psi_1)) (increase (total-cost) 1)))
 (:action closeobject
  :parameters ( ?a - agent ?l - location ?r - receptacle)
 ...
 (:action pickupobject
  ...
  :effect (and (not (inreceptacle ?o ?r)) (holds ?a ?o) (holdsany ?a) (not (objectatlocation ?o ?l)) (increase (total-cost) 1)))
 (:action putobject
  ...
  :effect (and (inreceptacle ?o ?r) (objectatlocation ?o ?l) (not (holds ?a ?o)) (not (holdsany ?a)) (increase (total-cost) 1)))
 (:action cleanobject
