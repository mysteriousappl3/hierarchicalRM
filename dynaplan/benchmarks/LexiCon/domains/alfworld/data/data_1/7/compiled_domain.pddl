(define (domain liftedtcore_alfred-domain)
 ...
 (:constants
   agent1 - agent
   location2 location3 location8 - location
 ...
 (:action gotolocation
  ...
  :effect (and (not (atlocation ?a ?lstart)) (atlocation ?a ?lend) (when (or (and (= ?a agent1) (= ?lend location2)) (and (atlocation agent1 location2) (not (and (= ?a agent1) (= ?lstart location2))))) (hold_0)) (when (or (and (= ?a agent1) (= ?lend location3)) (and (atlocation agent1 location3) (not (and (= ?a agent1) (= ?lstart location3)))) (and (= ?a agent1) (= ?lend location8)) (and (atlocation agent1 location8) (not (and (= ?a agent1) (= ?lstart location8))))) (seen_psi_1)) (increase (total-cost) 1)))
 (:action openobject
