(define (domain liftedtcore_alfred-domain)
 ...
 (:constants
   location5 location4 location8 - location
   agent1 - agent
 ...
 (:action look
  ...
  :effect (and (checked ?l) (when (or (= ?l location5) (checked location5) (atlocation agent1 location4)) (seen_psi_1)) (increase (total-cost) 1)))
 (:action inventory
  :parameters ( ?a - agent)
 ...
 (:action gotolocation
  ...
  :precondition (and (atlocation ?a ?lstart) (or (not (or (and (= ?a agent1) (= ?lend location8)) (and (atlocation agent1 location8) (not (and (= ?a agent1) (= ?lstart location8)))))) (seen_psi_1)))
  :effect (and (not (atlocation ?a ?lstart)) (atlocation ?a ?lend) (when (or (and (= ?a agent1) (= ?lend location8)) (and (atlocation agent1 location8) (not (and (= ?a agent1) (= ?lstart location8))))) (hold_0)) (when (or (checked location5) (and (= ?a agent1) (= ?lend location4)) (and (atlocation agent1 location4) (not (and (= ?a agent1) (= ?lstart location4))))) (seen_psi_1)) (increase (total-cost) 1)))
 (:action openobject
