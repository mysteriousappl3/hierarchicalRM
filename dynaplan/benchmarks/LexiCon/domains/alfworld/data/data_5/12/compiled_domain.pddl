(define (domain liftedtcore_alfred-domain)
 ...
 (:action look
  ...
  :effect (and (checked ?l) (when (or (= ?l location5) (checked location5) (atlocation agent1 location2)) (hold_4)) (increase (total-cost) 1)))
 (:action inventory
  :parameters ( ?a - agent)
 ...
 (and (atlocation ?a ?l_0) (receptacleatlocation ?r_0 ?l_0) (inreceptacle ?o ?r_0) (or (not (openable ?r_0)) (opened ?r_0))))) (holds ?a ?o)))
  :effect (and (checked ?o) (when (or (= ?o box_5) (checked box_5)) (hold_2)) (when (or (= ?o watch_1) (checked watch_1)) (hold_3)) (increase (total-cost) 1)))
 (:action gotolocation
  :parameters ( ?a - agent ?lstart - location ?lend - location)
  :precondition (and (atlocation ?a ?lstart))
  :effect (and (not (atlocation ?a ?lstart)) (atlocation ?a ?lend) (when (or (and (= ?a agent1) (= ?lend location5)) (and (atlocation agent1 location5) (not (and (= ?a agent1) (= ?lstart location5))))) (hold_0)) (when (or (and (= ?a agent1) (= ?lend location1)) (and (atlocation agent1 location1) (not (and (= ?a agent1) (= ?lstart location1))))) (hold_1)) (when (or (checked location5) (and (= ?a agent1) (= ?lend location2)) (and (atlocation agent1 location2) (not (and (= ?a agent1) (= ?lstart location2))))) (hold_4)) (increase (total-cost) 1)))
 (:action openobject
