    ...
    IEnumerator CallGeminiAPI(string prompt)
{
    ...
    string jsonRequest = $@"{{
        ""contents"": [
            {{
                ""role"": ""user"",
                ""parts"": [
                    {{ ""text"": ""You are an action generator VLM at a hierarchy level of H1 for given user instruction."" }}
                ]
            }},
            {{
                ""role"": ""user"",
    ...
    Debug.Log("[H1ActionGenerator] Sending Gemini API Request...");
    yield return request.SendWebRequest();
    Debug.Log("[H1ActionGenerator] Received API Response.");
    ...
    else
    {
        ...
        if (s < 0)
        {
            Debug.LogError("[H1ActionGen] start_flag not found in Gemini response.");
            yield break;
        }
        ...
        if (e < 0)
        {
            Debug.LogError("[H1ActionGen] end_flag not found in Gemini response.");
            yield break;
        }
        ...
        {
            Debug.LogError("[H1ActionGen] Extracted output between flags is empty.");
            yield break;
        }

        Debug.Log("[H1ActionGenerator] Gemini Output:\n" + content_output);

        // In the OpenAI version, you extract the main payload from content_output
        ...
        if (ms >= 0)
        {
            ...
            {
                Debug.LogError("[H1ActionGen] end_mapping not found in Gemini response.");
            }
        }
        else
        {
            Debug.LogError("[H1ActionGen] start_mapping not found in Gemini response.");
        }
