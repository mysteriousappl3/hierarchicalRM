...
public class InnerBot : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        replanVlmprompt = @"
            You are a misjudgement robot, and your main job is to determine whether the task list and code passed from 
            the decision bot are correct in the current environment. If they are correct, reply with YES. If they are incorrect,
            reply with NO and explain the reason behind your choice.

            An image of the current environmental information is provided.
    ...
    public IEnumerator verifyPlan(bool isStateDesc)
    {
        ...
        // TODO -- Based on the discussion to above questions, complete the remaining template for this function later
        // yield return StartCoroutine(CallDeepSeekAPI(prompt));
    ...
    IEnumerator CallGeminiAPI(string prompt)
{
    ...
    public IEnumerator CallReplanVlmApi(string imagePath)
    {
        ...
        {
            Debug.LogError($"[InnerBot ReplanVLM] Image file not found: {imagePath}");
            yield break;
        }

        byte[] imageData = File.ReadAllBytes(imagePath);
        string base64Image = Convert.ToBase64String(imageData);
        Debug.Log($"[InnerBot ReplanVLM] Loaded image from {imagePath}");
        ...
        // 3. Escape prompt
        replanVlmprompt = replanVlmprompt.Replace("{user_instruction}", main.userInstruction).Replace("{decision_bot_output}", decisionBot.output);
        ...
        string jsonRequest = $@"{{
            ...
            ""input"": [
                {{
                    ""role"": ""system"",
                    ""content"": ""You are a misjudgement robot, and your main job is to determine whether the task list and code passed from the decision bot are correct in the current environment.""
                }},
                {{
                    ""role"": ""user"",
                    ""content"": [
        ...
        Debug.Log("[InnerBot ReplanVLM] Sending API Request...");
        yield return request.SendWebRequest();
        Debug.Log("[InnerBot ReplanVLM] Received API Response.");

        // 6. Process response
        if (request.result != UnityWebRequest.Result.Success)
        {
            Debug.LogError($"[InnerBot ReplanVLM] API Request Failed: {request.error}\n{request.downloadHandler.text}");
            yield break;
        }

        string jsonResponse = request.downloadHandler.text;
        Debug.Log("[InnerBot ReplanVLM] Raw API response:\n" + jsonResponse);
        ...
        {
            Debug.LogError("[InnerBot ReplanVLM] No valid message block in response.");
            yield break;
        }
        ...
        {
            Debug.LogError("[InnerBot ReplanVLM] No output_text found!");
            yield break;
        }

        output = outputText.Trim();
        Debug.Log("[InnerBot ReplanVLM] Output:\n" + output);

        // Assign verdict to YES or NO only
        string verdictText = output.ToUpperInvariant();
        if (verdictText.Contains("YES"))
        {
            verdict = "YES";
        }
        else if (verdictText.Contains("NO"))
        {
            verdict = "NO";
        }
        else
        ...
        Debug.Log($"[InnerBot ReplanVLM] Verdict: {verdict}");
    }
