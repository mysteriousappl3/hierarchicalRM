"""Dynamic (n-level) hierarchy benchmark.

Runs the same Hanoi tasks as hanoi_benchmark.py through a pipeline where
planning and hierarchy composition are separate stages:
...
Results are written under mode "dynamic-hierarchy", so the existing
hierarchy / inner-outer / not-hierarchy baselines are untouched and directly
comparable.

Usage mirrors hanoi_benchmark.py:
    python benchmarking/dynamic_hierarchy_benchmark.py --task hanoi_3 --provider mock
"""
