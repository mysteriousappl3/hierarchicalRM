"""Flat-to-flat Tower of Hanoi benchmark runner.

Mirrors hanoi_benchmark.py (hierarchy / inner-outer / not-hierarchy modes)
against flat_hanoi_task_loader.FlatHanoiTask and flat_prompts.py instead of
the tower-to-tower task_loader.py / prompts.py, so tower framing
(source_peg/auxiliary_peg/target_peg, "using X as the auxiliary peg") never
reaches a flat task's prompts. See hanoi_benchmark.py for the mode
descriptions and replanning behavior, which are unchanged here.

Adds --jobs for parallel execution across the task list, since flat runs are
independent and there is no concurrency in hanoi_benchmark.py to build on.
"""
...
from flat_hanoi_scoring import score_direct_output, score_from_execution
from flat_hanoi_task_loader import available_tasks, load_task


BENCHMARK_DIR = Path(__file__).resolve().parent
DEFAULT_ENV_FILE = BENCHMARK_DIR / ".env"
RESULTS_DIR = BENCHMARK_DIR / "results_flat"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "independent (separate task, separate result directory), so this "
            "gives near-linear speedup over the default serial loop. Each "
            "worker still makes model calls one at a time within its own task."
        ),
    )
...
def main() -> int:
    ...
    def run_one(task_id: str) -> Dict[str, object]:
        # Each concurrent task gets its own client instance, since ModelClient
...
def structured_result_dir(row: Dict[str, object]) -> Path:
    ...
    # A microsecond + short uuid suffix, matching the scripted-blocks-world
    # result dirs, since --jobs runs tasks concurrently and the plain
    # second-granularity timestamp used by hanoi_benchmark.py would collide
    # across threads finishing in the same second.
    unique = f"{time.time_ns() // 1000 % 1_000_000:06d}_{uuid.uuid4().hex[:6]}"
    leaf = f"{row['task']}_{timestamp}_{unique}"
...
def print_summary(rows: List[Dict[str, object]], output_path: Optional[Path]) -> None:
    ...
    for row in rows:
        ...
        optimal_move_count = score["optimal_move_count"]
        optimal_move_count_display = "--" if optimal_move_count is None else str(optimal_move_count)
        print(
            f"{row['task']:<8}  "
            ...
            f"{optimal_move_count_display:<12}  "
